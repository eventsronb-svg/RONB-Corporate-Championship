...
Player photos are equally private: `team_players.photo_url` stores an object key, never a public URL. Photos are served through authenticated short-lived redirects, valid for 300 seconds, from `GET /orders/:id/items/:item_id/player-photos/:position` (the owning captain) and `GET /admin/orders/:id/items/:item_id/player-photos/:position` (organizers). Unauthorized requests, public `/teams`, and the logged-in captain of a different order receive no photo key. Organizers can correct a roster from the admin Teams page: `PATCH /admin/teams/:id/items/:item_id/profile` replaces member names, jersey sizes, captain position and photo keys, and `POST /admin/teams/:id/items/:item_id/player-photos/:position` uploads a replacement member photo. These edits are recorded in the order's audit log.
...
- Selecting sports uses full replacement. Registration accepts one required company name and **exactly one sport**. Registering for additional sports means creating a separate registration; the same Google account reuses the same payment code each time. The server copies that company name to the selected sport entry using the existing `team_name` storage field. Blank company names, missing or multiple sports, and inactive sports are rejected. Selections remain editable in `phone_captured` until invoicing.
...
- Sport capacity is reserved when invoicing, using ordered sport-row locks to prevent concurrent overbooking. `/sports` exposes `filled_slots` (reserved capacity: invoiced or paid orders) and `listed_slots` (completed profiles only); the selected public sport tab shows its listed/total count. Rejection, cancellation, and expiry release reservations. Resubmitting a rejected receipt must reacquire a slot. Admins can edit capacity, but cannot reduce it below current reservations. Blank capacity means unlimited.
- A payment request is idempotent and the account's code is never renewed in place; every order from the same captain shows the same code. Expired unpaid codes require revision. A rejected receipt can be resubmitted even after the original code expiry because a transfer is already under dispute.
...
## Per-account payment code migration

Apply `migrations/006_payment_code_per_user.sql` through `npm run migrate` before deploying the updated application. This adds `users.payment_code` (unique per account) and backfills it from the earliest code already on file for each captain, then drops the unique constraint on `payment_requests.unique_code` so the same account can reuse its code across separate single-sport registrations. The field stays `NULL` until a captain's first payment request, which generates and stores the code once.
