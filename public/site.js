...
function home() {
  ...
  main.innerHTML = `
    ...
    <section class="section" id="sports">${sectionHeader('A game for your team', 'Good colleagues. Great teammates.', 'Pick your sport, rally your people, and give the office something new to talk about.')}<div class="sport-grid">${data.sports.map((sport, index) => `<a class="sport sport-${sport.slug}" href="/register?focus=${encodeURIComponent(sport.slug)}"><div class="sport-top"><span class="sport-format">${esc(sport.format)}</span></div><h3>${esc(sport.name)}</h3><p>${esc(sport.description)}</p><span class="sport-link">Let's play</span><span class="sport-symbol" aria-hidden="true">${['⚽', '🔴', '🏀'][index]}</span></a>`).join('')}</div><p class="section-note">Registration fees and available places are shown when you sign in.</p></section>
