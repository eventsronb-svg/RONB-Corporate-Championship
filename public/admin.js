...
async function queue(params) {
  ...
  ${data.orders.length ? `<div class="table-scroll"><table><thead><tr><th>Captain / company</th><th>Amount</th><th>Payment code</th><th>Status</th><th>Submitted</th><th><span class="sub">Review</span></th></tr></thead><tbody>${data.orders.map((o) => `<tr><td><a class="strong" href="#order/${o.id}">${e(o.company_name || o.captain_name)}</a><span class="sub">${e(o.captain_name)} · ${e(o.teams.map((t) => `${t.team_name} · ${t.sport_name}`).join(' / '))}</span></td><td>${o.total_amount === null ? '—' : money(o.total_amount)}</td><td class="mono">${e(o.unique_code || 'Not issued')}</td><td>${status(o.status)}</td><td>${e(date(o.receipt_submitted_at))}</td><td><a href="#order/${o.id}" aria-label="Review ${e(o.company_name || o.captain_name)}">Open ↗</a></td></tr>`).join('')}</tbody></table></div>` : empty('You’re all caught up.', 'No registrations match these filters. New receipt submissions will appear here.')}
